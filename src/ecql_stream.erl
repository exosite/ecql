...
handle_call({query_async, Statement, Args, Consistency}, _From, State0) ->
  ...
  ,{reply, ok, State1#state{async_pending = Pending + 1, async_laststmt = Statement, async_start = os:timestamp()}}
;
...
handle_call({query_batch_async, Cql, ListOfArgs, Consistency}, _From, State) ->
  ...
  ,{reply, ok, State1#state{async_pending = Pending + 1, async_laststmt = Cql, async_start = os:timestamp()}}
;
...
log(ResponseOpCode, Body, State = #state{async_laststmt = Cql, async_start = Begin}) ->
   ecql_log:log(timer:now_diff(os:timestamp(), Begin), async, Cql, [])
